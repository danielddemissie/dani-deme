...
2. Manually download the theme git repository and unzip it in our site under `theme` directory or use `git clone`. - use this approach if you are satisfied with the current change and doesn't intend to follow the change of the parent (theme) repository.
